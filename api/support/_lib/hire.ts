...
export async function nextQuestion(orgId: string, v: VacancyRow, profile: any,
  ...
  const arc = [
    ...
    `№2 и №3 — углубление и ПРОВЕРКА опыта. После каждого громкого заявления (рост, план, цифры) задавай контрольный вопрос на деталь ВТОРОГО порядка: чем измеряли, какой инструмент, какой был средний чек, кто принимал решение. Выдумка и заученный текст сыплются именно на деталях; связный кандидат отвечает не задумываясь.`,
    ...
    `Предпоследний вопрос — ЗАКРЫТИЕ ТРЕБОВАНИЙ: пробеги по требованиям вакансии, которые ещё не прозвучали в разговоре (например: опыт с ресторанами/HoReCa, личный автомобиль, языки), и задай ОДИН короткий комбинированный вопрос сразу по 2-3 незакрытым: «И пара коротких уточнений: работали ли вы с ресторанной сферой, есть ли личный автомобиль?»`,
    `Остальные до №${total} — проба B2B-продаж и добор. Если опыт именно ПРОДАЖ ещё не подтверждён деталями — спроси про самую крупную сделку: сумма, длина цикла, кто был ЛПР, главное возражение и как закрыл. Управленец без продажного опыта здесь виден сразу — это не провал, но это надо зафиксировать честно.`,
    `Замеченное противоречие (ответ не сходится с анкетой или с более ранним ответом) вскрывай сразу, вежливым уточнением: «вы упоминали X, а сейчас Y — помогите понять».`,
  ].join('\n')
  // Роль ИМЕННО следующего вопроса вычисляет код: номера в общем списке
  // модель считает плохо и пропускала закрытие требований
  const nextNo = askedNo + 1
  let currentRole: string
  if (nextNo === 1) currentRole = 'поздоровайся, В ОДНОЙ фразе объяви план и задай вопрос про опыт из анкеты'
  else if (nextNo <= 3) currentRole = 'углубление и проверка опыта: контрольный вопрос на деталь второго порядка (чем меряли, инструмент, средний чек, кто решал)'
  else if (nextNo === Math.min(4, total - 3)) currentRole = 'продукт: сначала 1-2 предложения о том, что Delever — B2B SaaS по подписке (сайт и приложение для заказов, доставка, QR-меню), затем ОДИН сценарий из списка'
  else if (nextNo === Math.min(5, total - 2)) currentRole = 'мотивация: почему к нам, почему уходит'
  else if (nextNo === total - 1) currentRole = 'ЗАКРЫТИЕ ТРЕБОВАНИЙ: посмотри на требования вакансии, найди 2-3, которые ещё НЕ прозвучали в разговоре (например опыт с ресторанами/HoReCa, личный автомобиль, языки), и задай по ним ОДИН короткий комбинированный вопрос'
  else if (nextNo === total) currentRole = 'финальный добор: самое слабое или неясное место разговора; если ожидания по деньгам ещё не выяснены числом — спроси их'
  else currentRole = 'ожидания по деньгам числом, если не ясны; иначе — проба продаж: самая крупная сделка (сумма, цикл, ЛПР, возражение)'
  ...
  const system = [
    ...
    `Всего ${total} вопросов, это будет вопрос №${nextNo}.`,
    `РОЛЬ ИМЕННО ЭТОГО ВОПРОСА: ${currentRole}. Отступай от роли только ради переспроса пустого ответа.`,
    `Общий план интервью для контекста:`,
    arc,
...
export async function scoreCandidate(orgId: string, v: VacancyRow & { weights: any; threshold: number }, profile: any,
  ...
  const system = [
    ...
    `Оценивай компетентность по ПОДТВЕРЖДЁННЫМ деталям (цифры, инструменты, роли ЛПР, названия), а не по заявлениям: «строил весь процесс продаж» без единой детали — это 30-40, не 70. Противоречия между ответами и анкетой — в red flags.`,
    ...
    `В qualification пройди по КАЖДОМУ требованию вакансии по отдельности + добавь пункты «Ожидания в вилке» и «Опыт продаж (не только управления)».`,
    `Статусы качества СТРОГО: met — кандидат ПОДТВЕРДИЛ словами; unmet — слова кандидата ПРЯМО ПРОТИВОРЕЧАТ требованию (сказал «машины нет», назвал ожидания выше вилки). Если тему НЕ СПРАШИВАЛИ или кандидат её не касался — это ВСЕГДА unknown, никогда не unmet: «не упомянул» ≠ «не соответствует». Не ставь кандидату в вину вопросы, которых ему не задали.`,
  ...
  // Предохранитель: модель иногда ставит unmet за «не упомянул» вопреки
  // правилу. Крест — только при прямом противоречии; «не спрашивали» = «?»
  const NOT_ASKED = /не упомянул|не упоминал|не спрашивал|не выяснено|неясно|не прозвучал|нет информац|не указал/i
  const qualification = (Array.isArray(out.qualification) ? out.qualification : [])
    .map((q: any) => ({
      requirement: String(q?.requirement || ''),
      status: ['met', 'unmet', 'unknown'].includes(q?.status)
        ? (q.status === 'unmet' && NOT_ASKED.test(String(q?.note || '')) ? 'unknown' : q.status)
        : 'unknown',
      note: q?.note ? String(q.note) : undefined,
    }))
    .filter((q: any) => q.requirement)
  return {
    qualification,
    score,

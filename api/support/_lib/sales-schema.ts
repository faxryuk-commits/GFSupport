...
const SOURCE_SEED: Array<{ key: string; label: string; kind: string }> = [
  ...
  // Сделка, набранная менеджером в Amo. Раньше приезжала под ярлыком
  // «Заведён вручную» и читалась как «сейлз завёл её здесь»: люди видели
  // у себя карточки, которых не создавали, и не понимали, откуда они
  { key: 'amo_manual', label: 'Amo · завёл менеджер', kind: 'referral' },
...
export async function ensureSalesSchema(sql: SQL, orgId: string): Promise<void> {
  ...
  await seedBatch(sql, 'sales_pipelines',
    ...
    [
      [`spl_${orgId}_sales`, orgId, 'sales', 'Общая воронка', null, 'sales', 0,
        'Для сделок без территории и для тех, кто работает сразу по всем рынкам'],
      [`spl_${orgId}_uz`, orgId, 'sales_uz', 'Узбекистан', 'uz', 'sales', 1, null],
      [`spl_${orgId}_kz`, orgId, 'sales_kz', 'Казахстан', 'kz', 'sales', 2, null],
      [`spl_${orgId}_kg`, orgId, 'sales_kg', 'Кыргызстан', 'kg', 'sales', 3, null],
      [`spl_${orgId}_az`, orgId, 'sales_az', 'Азербайджан', 'az', 'sales', 4, null],
      [`spl_${orgId}_ge`, orgId, 'sales_ge', 'Грузия', 'ge', 'sales', 5, null],
      [`spl_${orgId}_cy`, orgId, 'sales_cy', 'Кипр', 'cy', 'sales', 6, null],

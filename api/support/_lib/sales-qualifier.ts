...
/** Что агент выясняет — подмножество квалификации, о котором уместно спросить в чате. */
const ASK_FIELDS: Array<{ key: string; label: string }> = [
  { key: 'pos', label: 'какая касса (POS-система)' },
...
async function qualify(sql: SQL, orgId: string, input: QualifierInput): Promise<void> {
  ...
  // 1. Факты сохраняем всегда — даже когда отвечать нельзя. Пустое и уже
  //    известное не перетираем: клиент мог ответить «не знаю»
  const patch: Record<string, string> = {}
  for (const f of ASK_FIELDS) {
    const v = String(verdict.extracted?.[f.key] || '').trim()
    if (v && !knownOf(f.key)) patch[f.key] = v.slice(0, 200)
  }
...
async function askModel(
  ...
  const system = [
    ...
    'Твоя работа: вежливо выяснять факты о заведении и коротко отвечать на простые вопросы о продукте.',
    'Правила ответа: не более 300 знаков; максимум один вопрос, и только из списка невыясненного;',

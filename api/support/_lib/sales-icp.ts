...
export function scoreIcp(input: IcpInput): IcpResult {
  ...
  // 15–29 — ниже порога плейбука, но не повод для штрафа: ответ «15+» из
  // анкеты давал +8, а точная цифра «20» — минус десять. Наказывали за
  // точность ответа
  else if (orders >= 15) add(`${orders} заказов/день — ниже порога`, 1)

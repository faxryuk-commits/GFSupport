...
❌ ПЛОХО — пересказ слов клиента + робо-речь про процесс:
Клиент: «Заказы не приходят на кассу, теряем деньги!»
→ «У вас критическая проблема с заказами, которые не поступают в систему. Я сразу эскалирую её администратору.»
(повторил клиента, рассказал про внутреннюю кухню, пользы ноль)

✅ ХОРОШО — короткое признание + действие + диагностический вопрос:
Клиент: «Заказы не приходят на кассу, теряем деньги!»
→ «Понял, это срочно — уже подключаю команду. Чтобы ускорить: касса/iiko сейчас в сети? Не идут все заказы или отдельные?»

✅ ХОРОШО — тот же язык, что у клиента (узбекский):
Клиент: «Buyurtmalar kassaga tushmayapti»
→ «Tushunarli, shoshilinch — jamoani jalb qildim. Kassa hozir tarmoqqa ulanganmi? Hamma buyurtmalar tushmayaptimi yoki ayrimlari?»

Правило из примеров: НЕ переписывай проблему обратно клиенту и НЕ объявляй «эскалирую администратору» — вместо этого дай конкретный шаг или уточняющий вопрос.
...
⚠️ Отвечай С УЧЁТОМ ВСЕЙ ПЕРЕПИСКИ выше, а не только последнего сообщения — оно часто продолжение мысли. Если для ответа не хватает контекста (последнее сообщение обрывочное «да/ок/вот»), смотри, на какой вопрос/тему оно отвечает.${alreadyGreeted ? '\n⚠️ Ты уже здоровался в этом диалоге — НЕ ЗДОРОВАЙСЯ снова.' : ''}
...
export async function executeDecision(
  ...
  if (decision.action === 'escalate') {
    // Клиенту — человечный текст (replyText), НЕ внутренний reasoning. Сам reasoning
    // уходит только команде в уведомлении ниже. Раньше клиент видел «🔴 Эскалация: <reasoning>»
    // — пересказ его же слов с тревожным значком, плюс сгенерированный replyText терялся.
    const clientMsg = decision.replyText?.trim()
      || 'Здравствуйте! Передал вашу проблему команде — уже разбираемся, скоро вернёмся с решением.'
    try {
      await clientSend(ctx.channelId, clientMsg)

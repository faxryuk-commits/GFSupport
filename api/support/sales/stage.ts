  ...
  if ((target.key === 'kp' || target.kind === 'won') && !specFilled && !body?.skipSpec) {
    return json({
      ...
      message: target.key === 'kp'
        ? 'Не хватает «Состава подключения» — это отдельный блок внизу '
          + 'карточки: касса, оплата, агрегаторы, доставка. Он войдёт в КП — '
          + 'клиент увидит, что именно ему подключат, а не общий прайс.'

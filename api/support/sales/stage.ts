...
async function handlerInner(req: Request): Promise<Response> {
  ...
  // На КП состав не требуем: сейлз собирает предложение по разговору, а
  // блокировка этапа полем из другого блока читалась как поломка. Перед
  // выигрышем — по-прежнему просим: без ТЗ отдел подключения начнёт
  // выяснять заново то, что сейлз уже знает
  if (target.kind === 'won' && !specFilled && !body?.skipSpec) {
    return json({
      blocked: true,
      needSpec: true,
      message: 'Перед выигрышем заполните «Состав подключения» — блок внизу '
        + 'карточки: касса, оплата, агрегаторы, доставка. Без него отдел '

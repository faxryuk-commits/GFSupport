...
| Шаг | Действие | Эффект |
|---|---|---|
| 0 | **СНАЧАЛА: полный wipe через POST /logout** | Чаще всего и решает — см. ниже |
| 1 | Ждём 30+ мин с момента последнего 401, бридж сам попробует | Бесплатно, IP может остыть |
| 2 | Пересоздаём Railway service (новый IP) | Новый IP вне anti-abuse |
...
> **Реальный кейс 2026-06-03**: 13 дней стойкий 401, QR не появлялся. Причина оказалась
> НЕ в IP, а в **битой полу-сессии**: `creds.json` с `registered=false`, но со старым
> noise-ключом (от номера, которым пытались привязаться pair-code'ом). WA отвергал именно
> этот флагнутый идентификатор с 401. **Полный wipe через `/logout` → новый noise-ключ →
> WA сразу выдал QR.** Поэтому шаг 0 (wipe) пробуем ДО смены IP.
>
> Wipe (секрет не светим в логи):
> ```bash
> SECRET=$(railway variables --kv | grep '^BRIDGE_SECRET=' | cut -d= -f2-)
> curl -s -X POST https://<bridge-host>/logout -H "Authorization: Bearer $SECRET"
> ```
> Через ~40 сек проверить `/qr` — если `qr present=true`, идти сканировать в UI.
> Если снова 401 (`initialRejectStreak` растёт) — тогда уже IP, шаги 2-3.

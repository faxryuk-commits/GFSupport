...
export function CallPhone({ phone, market, leadId, size = 'md', channels: withChannels, className = '' }: Props) {
  ...
  const toggleMenu = (e: React.MouseEvent) => {
    ...
    if (r) setRect({ top: r.bottom + 4, left: Math.min(r.left, window.innerWidth - 300) })
    setOpen(o => !o)
  }
  ...
  return (
    <span ref={boxRef} className={`inline-flex items-baseline gap-1 ${className}`}>
      ...
      {open && rect && createPortal(
        <div ref={popRef}
          style={{ position: 'fixed', top: rect.top, left: rect.left, width: 290, zIndex: 60 }}

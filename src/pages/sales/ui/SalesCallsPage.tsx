...
import { PhoneIncoming, PhoneOutgoing, Play, ChevronDown, ChevronRight } from 'lucide-react'
...
interface Stats {
  ...
  calls: Array<{
    ...
    /** Звонок коллеге: в ленте виден, в продажах не считается. */
    internal?: boolean; colleague?: string | null
  }>
}
type CallRow = Stats['calls'][number]

/**
 * Лента группируется по номеру: десять попыток на один телефон — одна строка
 * с «× 10», а не десять одинаковых. Раскрывается до попыток по клику.
 */
interface CallGroup {
  key: string
  number: string
...
function groupCalls(calls: CallRow[]): CallGroup[] {
  const map = new Map<string, CallGroup>()
  for (const c of calls) {
    const key = c.number.replace(/\D/g, '').slice(-9) || c.number || '?'
    let g = map.get(key)
    if (!g) {
      g = { key, number: c.number, calls: [], answered: 0, talkSec: 0, who: [], latest: c,
            internal: Boolean(c.internal), colleague: c.colleague || null }
      map.set(key, g)
    }
    g.calls.push(c)
    if (c.answered) { g.answered++; g.talkSec += c.talkSec }
    if (c.who && !g.who.includes(c.who)) g.who.push(c.who)
    if (!c.internal) g.internal = false
    if (new Date(c.at) > new Date(g.latest.at)) g.latest = c
  }
  return [...map.values()].sort((a, b) => new Date(b.latest.at).getTime() - new Date(a.latest.at).getTime())
}
...
export default function SalesCallsPage() {
  ...
  const groups = useMemo(() => groupCalls(data?.calls || []), [data])
      ...
      <Card title="Лента звонков" sub="последние звонки периода — с карточками и записями"
        ...
        {!groups.length ? (
          ...
          <div className="divide-y divide-gray-50">
            {groups.map(g => {
              const p = parsePhone(g.number)
              const many = g.calls.length > 1
              const open = openGroups.has(g.key)
              const lead = g.calls.find(c => c.leadId) || null
              const deal = g.calls.find(c => c.dealId) || null
              const c = g.latest
              ...
              return (
                <div key={g.key} className={`px-4 py-2.5 ${g.internal ? 'bg-gray-50/60' : ''}`}>
                  <div className="flex items-center gap-3 flex-wrap">
                    <span className={`flex-none w-7 h-7 rounded-full flex items-center justify-center ${
                      g.answered ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-500'}`}>
                      {c.direction === 'in'
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-baseline gap-2 flex-wrap">
                        <span className="text-[13px] font-medium text-gray-800 tabular-nums">
                          {p.valid ? p.pretty : g.number || 'номер скрыт'}
                        </span>
                        {many && (
                        ...
                        {g.internal && (
                          <span className="text-[10.5px] font-semibold px-1.5 py-0.5 rounded-md bg-slate-200 text-slate-700">
                            сотрудник{g.colleague ? ` · ${g.colleague}` : ''}
                          </span>
                        )}
                        <span className={`text-[11.5px] ${g.answered ? 'text-emerald-600' : 'text-red-500'}`}>
                          {g.answered
                            ? (many ? `${g.answered} разг. · ${fmtDur(g.talkSec)}` : `${c.talkSec} сек`)
                            : c.direction === 'in' ? (many ? 'все пропущены' : 'пропущен') : (many ? 'все недозвоны' : 'недозвон')}
                        </span>
                        {g.who.length > 0 && <span className="text-[11.5px] text-gray-400">· {g.who.join(', ')}</span>}
                      </div>
                      <div className="text-[11px] text-gray-400 flex items-center gap-2 flex-wrap">
                        <span>{many ? `последний ${fmtAt(c.at)}` : fmtAt(c.at)}</span>
                        {lead?.leadId && (
                          <Link to={`/sales/leads/${lead.leadId}`} className="text-blue-600 hover:underline truncate max-w-[220px]">
                            {lead.leadName || 'лид'}
                          </Link>
                        )}
                        {deal?.dealId && (
                          <Link to={`/sales/deals/${deal.dealId}`} className="text-violet-600 hover:underline truncate max-w-[220px]">
                            {deal.dealTitle || 'сделка'}
                          </Link>
                        )}
                        {!lead && !g.internal && g.number.replace(/\D/g, '').length >= 7 && (
                          <button onClick={() => createLead(g.number)} disabled={creating}
                    ...
                    {!many && c.answered && c.uuid && !g.internal && <CallInsight uuid={c.uuid} />}
                  </div>
                  {!many && playing?.uuid === c.uuid && (
                    <audio controls autoPlay src={playing.url} className="mt-2 w-full h-9" />
                  )}

                  {/* Попытки внутри группы: время, исход, кто звонил, запись */}
                  {many && open && (
                    <div className="mt-2 ml-10 divide-y divide-gray-100 border-l-2 border-gray-100 pl-3">
                      {g.calls.map((a, i) => (
                        <div key={a.uuid || i} className="py-1.5 flex items-center gap-2 flex-wrap text-[11.5px]">
                          <span className="text-gray-500 tabular-nums w-[92px]">{fmtAt(a.at)}</span>
                          <span className={a.answered ? 'text-emerald-600' : 'text-red-500'}>
                            {a.answered ? `${a.talkSec} сек` : a.direction === 'in' ? 'пропущен' : 'недозвон'}
                          </span>
                          {a.who && <span className="text-gray-400">· {a.who}</span>}
                          {a.uuid && (
                            <button onClick={() => listen(a.uuid!)} disabled={recBusy === a.uuid}
                              className="ml-auto flex items-center gap-1 px-2 py-0.5 rounded-md border border-gray-200
                                         text-gray-600 hover:border-emerald-300 hover:text-emerald-700 disabled:opacity-40">
                              <Play className="w-3 h-3" />{recBusy === a.uuid ? '…' : 'запись'}
                            </button>
                          )}
                          {a.answered && a.uuid && !a.internal && <CallInsight uuid={a.uuid} />}
                          {playing?.uuid === a.uuid && (
                            <audio controls autoPlay src={playing.url} className="w-full h-9 mt-1" />
                          )}
                        </div>
                      ))}
                    </div>
